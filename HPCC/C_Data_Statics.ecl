...
uniqueDeviceCountTime := TABLE(SortedDeviceTripFileTable,{deviceID,Cnt:= (DECIMAL7_2)(COUNT(GROUP)/3600)},deviceID);
...
uniqueDeviceCountTimeTrip := TABLE(uniqueDeviceTrip,{deviceID, CntTrip:= COUNT(GROUP),
                                                                    AvgTripTime:= (DECIMAL7_2)AVE(GROUP,cnt)/60},deviceID);
uniqueDeviceCountTimeTrip;
